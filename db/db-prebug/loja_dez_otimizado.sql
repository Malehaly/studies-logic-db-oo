...
-- sql é dividido em 3:
-- ddl - (create, drop)
-- dml - manipular informacao - "crud"(create, retrieve, update, delete) (insert)
--
...
CREATE SCHEMA loja_dez CHARACTER SET utf8 ;
USE loja_dez ;
-- if not exists ; default; = nao precisa
-- ```sao crases e nao aspas - define texto - erros com palavras reservadas e acentuações
...
CREATE TABLE tipo (
  ...
  PRIMARY KEY (id))
ENGINE = InnoDB;
-- unique direto no nome facilita a escrita; porém é mais recomendado usar um índice com Unique Index para saber o erro direto quando aparecer;
-- unsigned logo atras do valor numerico, se nao da erro;
...
CREATE TABLE IF NOT EXISTS `loja_dez`.`produto` (
  ...
  `tipo_id` SMALLINT UNSIGNED NULL REFERENCES tipo(id) COMMENT 'pode ter uma PF com valor nulo, por exemplo se a loja tivesse um novo produto que seu tipo não está cadastrado.\n',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
-- "on delete cascade" deleta tudo com a referencia a ser deletada. exemplo: deletar SP, apaga as cidades de SP.
-- "no action" só deleta o que pediu, repassa os erros;
-- nao precisa ter o index do FK, por default vai vir no action;
-- o default do foreign key talvez nao puxa o constraint , assim afeta a integridade física das tabelas (dpnde do sistema)
